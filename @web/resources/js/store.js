...
//   key: 'cryptomania-app', // The key to store the state on in the storage provider.
//   storage: localStorage, // or sessionStorage or any other storage you want to use
// });
...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    updateData(state) {
      ...
      if (state.providers.length <= 0) {
        axios.post('/api/data/providers').then(({data}) => {state.providers = data;});
      }
      
      axios.post('/api/data/currencies').then(({data}) => {
        state.currencies = data;
        if (!state.currency || data[state.currency] === undefined) {
        state.currency = data[Object.keys(data)[0]].id;
        }
      });
  ...
  actions: {
    ...
    login({commit}, credentials, callback = null) {
