      ...
      open(tab = null) {
        Bus.$emit('modal:new', {
          ...
          component: {
            ...
            computed: {
              ...mapGetters(['user', 'vip', 'currency', 'currencies', 'fiatView'])
            },
            methods: {
              makeDeposit() {
              ...
              clickMaxWithdraw() {
                // console.log('balance-------', this.currencies[this.currency]);
                this.withdrawAmount = this.fiatView?this.tokenToFiat(this.currencies[this.currency].price, this.currencies[this.currency].balance.real ? this.currencies[this.currency].balance.real : 0).toFixed(this.currency==='commerce_shibainu'?6:2):this.currencies[this.currency].balance.real;
              },
              loadTab() {
                if(this.isHistory) {
                    ...
                    <div class="wallet-content">
                      ...
                      <template v-if="tab === 'deposit'">
                        <div class="deposit-currencies">
                          <dropdown-currency :entries="Object.keys(currencies).map(e => currencies[e])"
                            :onSelect="(e) => { $store.dispatch('setCurrency', e.id) }" :select="currency"></dropdown-currency>

                          <dropdown-currency v-if="currencies[currency].chains.length > 0"
                            :entries="currencies[currency].chains.map(e => { return { name: e.name, id: e.id } })"
                            :onSelect="(e) => { chain = e; loadTab(); }"></dropdown-currency>
                        <template v-if="!currency.startsWith('local_')">
                          <div class="deposit-address">
                            ...
                            <div style="color: #ff774a; font-size: 12px; margin-top: 10px"><web-icon icon="fas fa-fw fa-info-circle"></web-icon> Your deposit must be sent on the {{currencies[currency].displayName}} network to be processed.</div>
                          </div>
                      ...
                      <template v-if="tab === 'withdraw'">
                        <div class="deposit-currencies">
                          <dropdown-currency :entries="Object.keys(currencies).map(e => currencies[e])"
                            :onSelect="(e) => { $store.dispatch('setCurrency', e.id); withdrawAmount=0 }" :select="currency"></dropdown-currency>
                        ...
                        <div class="withdraw-address">
                          ...
                          <div class="v">
                            <web-icon class="withdraw-icon" :icon="fiatView?'Image/img/EUR.svg':currencies[currency].icon"></web-icon>
                            <input :placeholder="$t('walletModal.withdrawAmount')" v-model="withdrawAmount" type="number" style="padding-left: 35px; padding-right: 65px;">
                            <span class="max-btn" @click="clickMaxWithdraw">Max</span>
                          </div>
                        </div>
                        <div class="notes">
...
<style lang="scss">
  ...
  .xmodal.user_wallet {
    ...
    input {
      ...
      // margin-right: 15px;
      border-radius: 5px;
      border: 1px solid #324468 !important;
    }
