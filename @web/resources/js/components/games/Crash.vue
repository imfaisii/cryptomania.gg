...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['theme', 'currency', 'fiatView'])
        },
        methods: {
            lerp(v0, v1, t) {
            ...
            updateMultiplier() {
                if(this.gameInstance.bettingType === 'manual' && this.placedBetThisRound && !this.crashed) {
                    if(!this.fiatView) 
                        $('.play-button').html(this.$i18n.t('general.take',
                         { value: (this.betValue * parseFloat(this.currentMultiplier)).toFixed(this.currency.startsWith('local_') ? 2 : 8) })).removeClass('disabled');
                    else 
                        $('.play-button').html(this.$i18n.t('general.take',
                         { value: this.tokenToFiat(this.currencies[this.currency].price, this.betValue * parseFloat(this.currentMultiplier)).toFixed(2) })).removeClass('disabled');
                }
            },
            startGame() {
