<template>
    <div class="wager-chip wager-selector os-host-flexbox">
        <overlay-scrollbars :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }">
            <div v-for="(chip, i) in data.chips" :key="i" :class="`chip ${active === i ? 'active' : ''}`" :data-display="chip[0].toFixed(currency.startsWith('local_') ? 2 : 3)" :data-value="currency.startsWith('local_') ? chip[0].toFixed(2) : chip[1].toFixed(3)" @click="click(chip, i)">
                {{ abbreviate(chip[0]) }}
            </div>
        </overlay-scrollbars>
    </div>
