...
<script>
    ...
    if (step !== this.pins + 1) {
        ...
        const animationCallback = function () {
            plinkoInstance.animationCallback($(this));
        };
        ...
        obj.removeAttr('heading').delay(this.speed / 10).queue(function () {
            ...
            // Display result after ball animation completes
            const totalAnimationTime = plinkoInstance.speed * (plinkoInstance.pins + 1);
            setTimeout(() => {
                const response = {}; // You need to define the response object here
                Bus.$emit('game:customHistory:add', {
                    text: `${parseFloat(response.game.multiplier).toFixed(2)}x`,
                    style: `background: ${colors[plinkoInstance.pins][response.game.data.bucket][0]}; border-bottom: 1px solid ${colors[plinkoInstance.pins][response.game.data.bucket][1]}`,
                    seed: {
            ...
            reset() {
                ...
                for (let i = 0; i <= this.pins; i++) {
                    for (let j = 0; j <= i; j++) {
                        ...
                        let width = (isBucket ? (100 * s) * 2.8 : 100 * s);
                        let css = {
                            position: 'absolute',
                            top: (100 * y) + '%',
                            left: (100 * x) + '%',
    ...
    // Calculate total animation time (drop + ball animation)
    const totalAnimationTime = this.speed + this.speed * (this.pins + 1);

    // Show result after totalAnimationTime
    setTimeout(() => {
        Bus.$emit('game:customHistory:add', {
            text: `${parseFloat(response.game.multiplier).toFixed(2)}x`,
    ...
    if (this.gameInstance.bettingType === 'auto') {
        ...
        setTimeout(() => this.updateGameInstance((i) => i.playTimeout = false), 300);
    }
},
            getClientData() {
                return {
...
<style lang="scss">
    ...
    20% { transform: translate(-50%, calc(-50% + 30px)) scale(1); }
    40% { transform: translate(-50%, calc(-50% + 60px)) scale(1); }
    60% { transform: translate(-50%, calc(-50% + 90px)) scale(1); }
    80% { transform: translate(-50%, calc(-50% + 120px)) scale(1); }
    100% { transform: translate(-50%, calc(100% + 50px)) scale(0.8); }
}
