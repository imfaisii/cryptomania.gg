    ...
    <div class="fixed">
      <div class="chatHeader">
        ...
        <select class="languageSelector" @change="setLanguage(language)" v-model="language">
          ...
          <option :selected="locale === 'ar'" value="ar">🇦🇪&emsp;عربي</option>
        </select>
        <div @click="closeChat" style="display: flex; align-items: center; font-size: 1.2rem;">
          ...
          <div v-if="message.type === 'message'" :key="j" :class="`message from-${message.user.access}`"
            ...
            <div class="avatar">
              ...
              <div class="message-content">                
                <div class="content">                
                  <div class="firstMessage"><span @click="openUserModal(message.user._id)" class="name">
                  {{ formatName(message.user.name) }}:
                </span><span>{{ message.data }}</span></div>
                </div>
              </div>
            </div>
      ...
      <div class="message-send">
        ...
        <div class="column">
          ...
          <input @keyup.enter="sendChatMessage" v-model="message" class="text-message"
                    :placeholder="$t('general.chat.enter_message')" style="height: 60px;">
        </div>
              ...
              <div class="content">
                <overlay-scrollbars :options="{ scrollbars: { autoHide: 'leave' }, className: 'os-theme-thin-light' }">
                  ...
                  <template v-else-if="emojiCategory === 'emoji'">
                    <div class="emoji" v-for="emoji in unicodeEmoji"
        ...
        <div class="column" style="height: 48px;">
          ...
          <div @click="openChatRuleModal" style="margin-right: 0; margin-left: auto; cursor: pointer; display: flex; align-items: center;">
            <i data-v-32aed0c7="" class="fal fa-question-circle" style="margin-right: 5px;"></i> Chat rules
          </div>
        </div>
      </div>
    </div>
...
<script>
 ...
 //     setInterval(() => {
//  this.whisper('OnlineUsers').then((response) => {
//    const baseCount = response ? response.length : 0;
//    const randomOffset = Math.floor(Math.random() * 41) + 80; // Random number between 80 and 120
//    this.onlineUsers = baseCount + randomOffset;
//  }).catch(() => {
//    const randomOffset = Math.floor(Math.random() * 41) + 80;
//    this.onlineUsers = randomOffset; // Fallback to random offset if there's an error
//  });
