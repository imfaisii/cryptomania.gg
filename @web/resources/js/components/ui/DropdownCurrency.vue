  ...
  <div class="walletExchangeSelectors">
    <div class="walletExchangeSelector">
      <div class="wesContainer" @click="expand = !expand" :style="`${expand?'border: 1px solid #6f30d7;':'border: 1px solid #324468;'}`">
        ...
        <div class="balance">          
          <unit :fiat="fiatView" :to="selected.id" :value="demo ? selected.balance.demo : selected.balance.real"></unit>
        </div>
        <div class="icon"><web-icon :icon="`fal fa-fw fa-chevron-${expand?'up':'down'}`"></web-icon></div>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['user', 'demo', 'unit', 'currency', 'currencies', 'fiatView'])
    },
    created() {
      if(!this.select)
