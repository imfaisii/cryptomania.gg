...
use App\Games\Kernel\ThirdParty\WorldSlotGame\WorldSlotGame;
use App\Games\Kernel\ThirdParty\WorldSlotGame\WorldSlotGameGame;
use App\Games\Kernel\ThirdParty\Slotegrator\Slotegrator;
use App\Games\Kernel\ThirdParty\Slotegrator\SlotegratorSports;
use App\Games\Kernel\ThirdParty\Slotegrator\SlotegratorGame;
...
Route::prefix('data')->group(function () {
  ...
  Route::post('/popularGames', function() {
    ...
    foreach (\App\Models\GameStats::orderBy('launches', 'desc')->limit(100)->get() as $item) {
  ...
  Route::post('/games', function () {
    ...
    foreach (Game::list() as $game) {
      ...
      $stats = \App\Models\GameStats::where('game', $game->metadata()->id())->first();
      ...
      $games[] = [
        ...
        'popularity' => $stats == null ? 0 : $stats->launches
      ];
    }
  ...
  Route::post('/providers', function () {
    if (Cache::has('game:providers')) {
      $providers = Cache::get('game:providers');
    ...
    Cache::put('game:providers', $providers, Carbon::now()->addDays(1));
    return APIResponse::success($providers);
  });
...
Route::middleware('auth:sanctum')->prefix('wallet')->group(function () {
  Route::post('exchange', function (Request $request) {
    ...
    if ($amount < 0.001) return APIResponse::reject(2, 'Invalid amount');
...
Route::middleware('auth:sanctum')->prefix('settings')->group(function () {
  ...
  Route::post('avatar', function (Request $request) {
    $request->validate([
      'image' => 'required|image|mimes:jpeg,png,jpg,svg|max:16064'
    ]);
...
Route::post('/slotegratorCallback', function (Request $request) {
  // $provider = ThirdPartyGame::findProvider('external:sg');
  // if ($provider === null) {
  //     // Log an error or handle the missing provider case
  //     Log::error('Provider not found for external:sg');
  //     return response()->json(['error' => 'Provider not found'], 404);
  // }
// Log:info('slotegrator-callback-body', $request->all());
  return (new SlotegratorGame('external:sg'))->processCallback(
      $request->all(),
      $request->all()['action'] ?? '',
...
// Route::get('/slotegratorSelfValidate', function (Request $request) {
//   (new Slotegrator())->sefValidate();
// });
...
Route::post('/websiteName', function() {
  return APIResponse::success([
    'name' => Settings::get('Website Name', 'Cryptomania') ?? 'Cryptomania'
  ]);
});

Route::post('/getSportsbook', function(Request $request) {
  // Log::info('slotegrator-sports-request');
  // Log::info($request['user']);
  return (new SlotegratorSports())->getSportsURL($request['user']);
});
