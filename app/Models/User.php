...
class User extends \Jenssegers\Mongodb\Auth\User {
  ...
  protected $fillable = [
    ...
    'isDemoLimitationsIgnored', 'isPhoenixGamblingManager', 'game_token'
  ];
  ...
  public $hidden = [
    ...
    'wallet_trx_private_key', 'wallet_native_xmr', 'game_token'
  ];
  ...
  public function setGameToken($token) {
...
class UserBalance {
  ...
  public function get(): float {
    ...
    $value = 0;
    if (isset($this->user->{$this->getColumn()}) && $this->user->{$this->getColumn()} instanceof Decimal128) {
      // Extract the float value from the Decimal128 object
      $value = floatval($this->user->{$this->getColumn()}->jsonSerialize()['$numberDecimal']);
    } else {
      // If the value is not set or not a Decimal128 object, use the default value
      $value = floatval($this->minValue);
    }
  ...
  public function add(float $amount, array $transaction = null, int $delay = 0, string $serviceId = null, string $serviceType = null) {
    // Log::info('game-update-balance--------------: ' . $serviceId . '//' . $this->get() . '////' . $amount);
    $this->user->update([
      $this->getColumn() => new Decimal128(strval($this->get() + $amount))
    ]);
    // Log::info('game-update-balance-updated--------------: ' . $serviceId . '//' . $this->get() . '////' . $amount);
